...
'''
GitHub Repo: https://github.com/facebookresearch/VideoPose3D/tree/main
'''
...
class TemporalModelBase(nn.Module):
    ...
    def __init__(self, num_joints_in, in_features, num_joints_out,
        ...
        self.expand_bn = nn.BatchNorm1d(channels, momentum=0.001)
...
class TemporalModelOptimized1f(TemporalModelBase):
    ...
    def _init_weights(self, pretrained=''):
        ...
        direction_chk = torch.load(pretrained, map_location=lambda storage, loc: storage)
        need_init_state_dict = {}
        for name, m in direction_chk['model_pos'].items():
...
if __name__ == '__main__':
    model_pos = TemporalModelOptimized1f(
        17, 2, 17,
        filter_widths=[3,3,3,3], dropout=0.25, channels=1024
    )
    
    import torch
    x = torch.zeros(32,81,17,2)
    model_pos(x)
